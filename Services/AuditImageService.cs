...
public sealed class AuditImageService
{
    ...
    public byte[] RenderSvg(AuditSnapshot snapshot)
    {
        ...
        sb.AppendLine(Escape($"participant={snapshot.Participant.Id};email={snapshot.Participant.Email};round={snapshot.Round.Id};generatedAt={snapshot.GeneratedAt:O};hash={snapshot.ProofHash}"));
        sb.AppendLine("</metadata>");
        ...
        sb.AppendLine(Text(64, 234, $"Rodada: {snapshot.Round.Name} | Gerado em UTC: {snapshot.GeneratedAt:yyyy-MM-dd HH:mm:ss}", 18, "#607166", 600));
        ...
        foreach (var line in snapshot.Predictions)
        {
            ...
            sb.AppendLine(Text(830, y, line.SavedAt.ToUniversalTime().ToString("MM-dd HH:mm"), 15, "#607166", 600));
            sb.AppendLine(Text(1010, y, line.SubmittedAt.ToUniversalTime().ToString("MM-dd HH:mm"), 15, "#607166", 600));
            y += rowHeight;
        }
